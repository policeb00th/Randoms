...
def coldet(img):
    ...
    cropped = resized[10:20, :]
    cropped1 = resized[:, 10:20]
    #cv2.imshow("cropped", cropped)
    #cv2.imshow("cropped1",cropped1)
    height, width, _ = np.shape(cropped)
    height, width, _ = np.shape(cropped1) 

    # reshape the image to be a simple list of RGB pixels
    image = cropped.reshape((height * width, 3))
    image1 = cropped1.reshape((height * width, 3))


    # we'll pick the 5 most common colors
    num_clusters = 2
    clusters = KMeans(n_clusters=num_clusters)
    clusters1 = KMeans(n_clusters=num_clusters)
    clusters.fit(image)
    clusters1.fit(image1)

    # count the dominant colors and put them in "buckets"
    histogram = make_histogram(clusters)
    # then sort them, most-common first
    combined = zip(histogram, clusters.cluster_centers_)
    combined = zip(histogram, clusters1.cluster_centers_)
    ...
    for index, rows in enumerate(combined):
        ...
        #if {rgb} == (np.hstack(bars),np.array(85, 50, 0),np.array(130,255,255)) :
        #if {rgb} <= np.array(85, 50,0) and {rgb}>= np.array(130,255,255):
            #img = colour(img, (85, 50, 0), (130, 255, 255))
            #print("blue")


        #print(f'  HSV values: {hsv}')
        
        x.append(coldetect.col_detect(hsv))        
